...
bool mpu9250_spi::init(int low_pass_filter){
    uint8_t i = 0;
...
void mpu9250_spi::read_acc()
{
    ...
    for(i=0; i<3; i++) {
        ...
        accelerometer_data[i]=data/16383.75;
    }
...
void mpu9250_spi::calibrateMPU(float* output1, float* output2){
        uint8_t data[12];
        ...
        WriteReg( 0x38, 0x00);   // Disable all interrupts
        WriteReg( 0x23, 0x00);      // Disable FIFO
        WriteReg( 0x6B, 0x00);   // Turn on internal clock source
        WriteReg( 0x24, 0x00); // Disable I2C master
        WriteReg( 0x6A, 0x00);    // Disable FIFO and I2C master modes
        WriteReg( 0x6A, 0x0C);    // Reset FIFO and DMP
        wait(0.015);
        ...
        WriteReg( 0x6A, 0x40);   // Enable FIFO
        WriteReg( 0x23, 0x78);     // Enable gyro and accelerometer sensors for FIFO (max size 512 bytes in MPU-9250)
        wait(0.04); // accumulate 40 samples in 80 milliseconds = 480 bytes

        WriteReg( 0x23, 0x00);        // Disable gyro and accelerometer sensors for FIFO
        ReadRegs( 0x72,data,2); // read FIFO sample count
